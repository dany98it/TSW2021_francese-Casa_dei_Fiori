			...
			<div>
				<form method="post" action="modificaItem">
					<input value="<%= i.getId() %>" name="id" hidden="false"> <label
						for="name">Nome:</label><br> <input
						value="<%= i.getNome() %>" id="name" name="name" type="text"
						maxlength="20" required placeholder="inserire nome"
					...
					<br> <label for="tag">tag:</label><br> <input id="tag"
						name="tag" type="text" data-role="tagsinput" placeholder="tag,..."
						onchange="prewievTag('tag','tagItem')"> <label
						for="caratterisiche">caratterisiche:</label><br> <input
						id="caratterisiche" name="caratterisiche" type="text"
						data-role="tagsinput" placeholder="caratterisica:valore,..."
						onchange="prewievTag('caratterisiche','caratterisicheItem')">
					<label for="price">Prezzo:</label><br> <input
						...
						value="<%= i.getSconto() %>" id="sconto" name="sconto"
						type="number" min="0" value="0" required
						onchange="prewievPrezzo('prezzoItem')"><br> <label
						for="tipo">Tipo:</label><br> <select name="tipo" id="tipo">
